...
- **Date fields** ([DateField.tsx](src/components/DateField.tsx)) are drawn by the app, because a native date input takes its format from the device language (not the page), has an unstyleable arrow, and overflows its container on iOS. Dates always read `dd. mm. yyyy`. On touch devices an invisible native date input covers the field, so a tap opens the system picker and screen readers get a native date control. Elsewhere it is a text field that accepts `15.4.2026`, `15. 04. 2026`, `15/4/2026` or `15042026`, with a button that opens the native calendar.
